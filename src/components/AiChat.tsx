...
export default function AiChat() {
  ...
  return (
    <div className="bg-slate-900/60 backdrop-blur-sm border border-slate-700 rounded-2xl shadow-2xl overflow-hidden flex flex-col h-[70vh] max-h-[640px]">
      {/* Messages */}
            ...
            <div className="flex flex-wrap justify-center gap-2 max-w-2xl">
              {SUGGESTED_QUESTIONS.map((q) => (
                <button
                  key={q}
                  onClick={() => send(q)}
                  className="text-sm text-purple-200 bg-purple-500/10 hover:bg-purple-500/20 border border-purple-500/30 rounded-full px-4 py-2 transition-colors"
                >
                  {q}
                </button>
              ))}
      ...
      <form
        ...
        className="border-t border-slate-700 p-3 sm:p-4 flex gap-2 bg-slate-900/80"
      >
        <input
          value={input}
          ...
          maxLength={500}
          disabled={loading}
          className="flex-1 bg-slate-800 border border-slate-700 focus:border-purple-500 rounded-lg px-4 py-3 text-white placeholder-gray-500 outline-none transition-colors disabled:opacity-60"
        />
        <button
          type="submit"
          disabled={loading || !input.trim()}
          className="bg-purple-600 hover:bg-purple-500 disabled:bg-slate-700 disabled:cursor-not-allowed text-white font-semibold px-5 rounded-lg transition-colors"
        >
          Send
        </button>
      </form>
