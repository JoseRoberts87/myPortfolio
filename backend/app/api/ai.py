...
per-IP hourly rate limit (Redis, fail-open) and request-size guards to bound
OpenAI cost.
"""
...
from app.schemas.ai import ChatResponse, ChatRequest
...
async def chat(payload: ChatRequest, request: Request) -> ChatResponse:
    if not settings.AI_CHAT_ENABLED or not rag_service.enabled:
        raise HTTPException(
            ...
            detail="The AI assistant is not configured. Set OPENAI_API_KEY to enable it.",
        )
